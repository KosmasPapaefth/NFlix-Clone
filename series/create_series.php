...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["SeriesName"]))) {
        $SeriesName_err = "Please enter a SeriesName.";
    } else {
    ...
    if (empty(trim($_POST["Episodes"]))) {
        $Episodes_err = "Please number of episodes.";
    } else {
    ...
    if (empty(trim($_POST["Seasons"]))) {
        $Seasons_err = "Please number of seasons.";
    } else {
    ...
    if (empty($moviename_err) && empty($Duration_err) && empty($Description_err)  && empty($Year_err) && empty($Category_err) && empty($Actors_err) && empty($Directors_err) && empty($ScreenWriter_err) && empty($Musicians_err) && empty($Soundtrack_err)) {
        ...
        $sql = "INSERT INTO series (SeriesName, Episodes , Seasons ,Duration, Description , Year , Category) VALUES (?, ? , ?, ? , ? , ? ,?)";
        $sql2 = "INSERT INTO creditsseries (Actors, Directors, ScreenWriter, Musicians, Soundtrack, SeriesId  ) VALUES(?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "siiisis", $param_SeriesName, $param_Episodes, $param_Seasons, $param_Duration, $param_Description, $param_Year, $param_Category);
        ...
        if ($stmt2 = mysqli_prepare($link, $sql2)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt2, "sssssi", $param_actors, $param_directors, $param_screenwriter, $param_musicians, $param_soundtrack, $param_seriesId);
...
<body>
    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5">Create Series</h2>
                    <p>Please fill this form and submit to add series record to the database.</p>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group">
                            <label>Numbers of Episodes</label>
                        ...
                        <div class="form-group">
                            <label>Numbers of Seasons</label>
